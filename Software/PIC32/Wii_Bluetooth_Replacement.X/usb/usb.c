...
#endif
    uart_transmit("Endpoint IN stall", 1);
}
...
static inline int8_t handle_standard_control_request()
{
	...
	if (setup->bRequest == GET_DESCRIPTOR &&
		...
		if (descriptor == DESC_DEVICE) {
			uart_transmit("Get Descriptor for DEVICE", 1);
...
#endif
			{
				...
				int16_t len;
				{
					...
					if (len < 0) {
						stall_ep0();
						uart_transmit("Unsupported string descriptor requested", 1);
					}
					else
			...
			if (len < 0) {
				stall_ep0();
				uart_transmit("Unsupported descriptor requested", 1);
			}
			else
				start_control_return(desc, len, setup->wLength);
#else
			...
			stall_ep0();
			uart_transmit("Unknown Descriptor", 1);
			uart_transmit_val(descriptor, 4, 1);
#endif
		}
	}
		...
		uart_transmit("Set configuration to ", 0);
		uart_transmit_val(req, 4, 1);
	}
	else if (setup->bRequest == GET_CONFIGURATION) {
		/* Return the current Configuration. */
		uart_transmit("Get Configuration. Returning: ", 0);
		uart_transmit_val(g_configuration, 4, 1);

		start_control_return(&g_configuration, 1, setup->wLength);
	}
	else if (setup->bRequest == GET_STATUS) {

		uart_transmit("Get Status (dst, index): ", 1);
		uart_transmit_val(setup->REQUEST.destination, 4, 1);
		uart_transmit_val(setup->wIndex, 4, 1);
		...
		if (setup->REQUEST.destination == 0 /*0=device*/) {
			...
			stall_ep0();
			uart_transmit("Stalling. Status Requested for destination: ", 0);
			uart_transmit_val(setup->REQUEST.destination, 4, 1);
		}
		...
		int8_t ret;
		uart_transmit("Get Interface", 1);
		uart_transmit_val(setup->bRequest, 4, 1);
		uart_transmit_val(setup->REQUEST.destination, 4, 1);
		uart_transmit_val(setup->REQUEST.type, 4, 1);
		uart_transmit_val(setup->REQUEST.direction, 4, 1);
		...
		uint8_t stall = 1;
		if (setup->REQUEST.destination == 0/*0=device*/) {
			uart_transmit("Set/Clear feature for device", 1);
		...
		uart_transmit("unsupported request (req, dest, type, dir) ", 1);
		uart_transmit_val(setup->bRequest, 4, 1);
		uart_transmit_val(setup->REQUEST.destination, 4, 1);
		uart_transmit_val(setup->REQUEST.type, 4, 1);
		uart_transmit_val(setup->REQUEST.direction, 4, 1);
...
static inline void handle_ep0_out()
{
	...
	if (ep0_data_stage_direc == 1/*1=IN*/) {
        ...
        //uart_transmit("Control transfer status complete (EP0 OUT handle)". 1);
		 ...
		 */
        //uart_transmit("Control transfer data received", 1);

		if (ep0_data_stage_out_buffer) {
            //uart_transmit("Control transfer data received", 1);
            //uart_transmit("Buf: ", 0);
            //uart_transmit_val(ep0_data_stage_out_buffer, 4, 1);
            //uart_transmit("Remaining data len: ", 0);
            //uart_transmit_val(ep0_data_stage_buf_remaining, 4, 1);
            ...
            //uart_transmit("OUT data: ", 0);
            //uart_hexdump(ep0_data_stage_out_buffer, pkt_len);
#else
			memcpy(ep0_data_stage_out_buffer, ep0_buf.out, bytes_to_copy);
#endif
...
static inline void handle_ep0_in()
{
	...
	if (ep0_data_stage_buf_remaining) {
        ...
        //uart_transmit("IN data: ", 0);
        //uart_hexdump(ep0_data_stage_in_buffer, bytes_to_send);
		...
		if (ep0_data_stage_direc == 0/*OUT*/) {
            ...
            //uart_transmit("Control transfer status complete (EP0 IN handle)", 1);
			if (ep0_data_stage_callback)
				ep0_data_stage_callback(1/*true*/, ep0_data_stage_context);
			reset_ep0_data_stage();
		}
...
void usb_service(void)
{
	if (SFR_USB_RESET_IF) {
		...
		usb_init();
		CLEAR_USB_RESET_IF();
		//uart_transmit("USB Reset", 1);
	}
	
	if (SFR_USB_STALL_IF) {
        //uart_transmit("USB Stall", 1);
	...
	if (SFR_USB_TOKEN_IF) {
		...
		if (SFR_USB_STATUS_EP == 0 && SFR_USB_STATUS_DIR == 0/*OUT*/) {
            //uart_transmit("OUT transaction received", 1);
			...
			if (pid == PID_SETUP) {
				...
				uart_transmit("Unsupported PID. Stall.", 1);
				stall_ep0();
			}
            ...
            //uart_transmit("IN transaction complete", 1);
			...
			if (SFR_USB_STATUS_DIR == 1 /*1=IN*/) {
				...
				//uart_transmit("IN transaction complete on non-EP0.", 1);
				...
				//uart_transmit("OUT transaction received on non-EP0", 1);
			...
			uart_transmit("Transaction completed for unknown endpoint", 1);
		}
...
#endif
        //uart_transmit("Start-of-frame interrupt", 1);
		CLEAR_USB_SOF_IF();
	}
